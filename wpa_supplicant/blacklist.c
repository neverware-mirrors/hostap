					 ...
					 const u8 *bssid)
{
	...
	if (wpa_s->current_ssid && wpa_s->current_ssid->blacklist_out_of_date) {
		wpa_printf(MSG_INFO, "Configs for currently active SSID have changed, "
			"clearing blacklist.");
		wpa_blacklist_clear(wpa_s);
		wpa_s->current_ssid->blacklist_out_of_date = 0;
		return NULL;
	}
...
int wpa_blacklist_add(struct wpa_supplicant *wpa_s, const u8 *bssid)
{
	...
	if (e) {
		...
		wpa_printf(MSG_INFO, "BSSID " MACSTR " blacklist count "
			   "incremented to %d, blacklisting for %d seconds",
	...
	wpa_s->blacklist = e;
	wpa_printf(MSG_DEBUG, "Added BSSID " MACSTR " into blacklist, blacklisting "
		   "for %d seconds", MAC2STR(bssid), e->timeout_secs);
...
/**
 * wpa_blacklist_is_blacklisted - Check the blacklist status of a BSSID
 * @wpa_s: Pointer to wpa_supplicant data
 * @bssid: BSSID to be checked
 * Returns: count if BSSID is currently considered to be blacklisted, 0 otherwise
 */
